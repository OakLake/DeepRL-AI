...
with graph.as_default():
        ...
        with tf.name_scope('Training'):
            ...
            loss = tf.reduce_mean(tf.square(tf_Y-logits))
...
with tf.Session(graph=graph) as session:
    ...
    D = 100000
    max_trial = 1000
    batch_size = 32 # ~0.01% of memory to train on each time step >> for 1 full episode of 1000 trials whole memory retrain

    for episode in range(10000):
        print('========')
        no_op_max = 10
        no_op = np.random.randint(10)
        epsilon *= eDecay
        state0 = env.reset()
        rtn = 0
        t = -1
        if (episode%50 ==0) and (render_FLAG):
