...
with graph.as_default():
        ...
        A1 = tf.matmul(tf_X,W1) + B1
        Relu = tf.nn.relu(A1)
        A2 = tf.matmul(Relu,W2) + B2
        Relu = tf.nn.relu(A2)
...
with tf.Session(graph=graph) as session:
    ...
    for episode in range(5000):
        ...
        # for t in range(500):
            ...
            action_prob = session.run([a_prob],feed_dict = {tf_X:[state0]})[0][0]
        ...
        feed_dict = {tf_X:batch_X,tf_Y:batch_Y,tf_discounted_reward_normd:batch_RD}
